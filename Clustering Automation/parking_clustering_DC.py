...
class Cluster:
    ...
    def load_data(self, geofence_file, neighborhood_file):
        ...
        with open('/Users/BrandonHall/Documents/GitHub/SUMDScrapeAndAnalysis/Clean_DC_CSVs/DC_Test.csv', 'r') as csv_file:
        ...
        # load geofence
        print("LOADING DC GEOFENCE AND NEIGHBORHOOD FILES")
        shpfile = ogr.Open(geofence_file)
        shapes = shpfile.GetLayer(0)
        self.facil_boundary = shapes.GetFeature(0)
        ...
        print("DONE LOADING DC GEOFENCE AND NEIGHBORHOOD FILES\n")
        #
    ...
    def clustering(self, facilities_file, csv_file, **kwargs):
        ...
        print("DONE OPENING DC FACILITIES FILE\n")

        # load train data file
        print("OPENING TRAINING DATA FILE")
        with open('Data Processing Files/Washington_DC_TrainingDataSample2.pkl', 'rb') as train_pickle:
            train_data = pickle.load(train_pickle)
        print("DONE OPENING TRAINING DATA FILE\n")

        # load test data file
        print("OPENING TESTING DATA FILE")
        with open('Data Processing Files/Washington_DC_TestingDataSample2.pkl', 'rb') as test_pickle:
        ...
        min_samples = [int(len(train_data) * 0.001), int(len(train_data) * 0.002), int(len(train_data) * 0.003),
                       int(len(train_data) * 0.004), int(len(train_data) * 0.005)]
        # **********************************************************************

        print("PERFORMING TRAINING/TESTING FOR CLUSTERING ALGORITHM")
        # *****WILL HAVE TO CHANGE THE PARAMETERS AND FUNCTION VARIABLES DEPENDING ON YOUR CLUSTERING ALGORITHMS
        # REQUIREMENTS*****
        for eps, smpls in product(kwargs['epsilon'], min_samples):
            try:
                # perform clustering using train data
                # ********CHANGE THIS PART OF CODE TO BEST FIT YOUR CLUSTERING ALGORITHMS' REQUIREMENTS*****
                clustering_i = DBSCAN(eps=eps, min_samples=smpls, metric='euclidean', n_jobs=-1)
                ...
                test_results_graph = (eps, smpls, graph_buffer_capture, len(train_unique_labels_i))
        ...
        csv_writer.writerow(('Data', 'Epsilon', 'Min Pts', 'Performance (buffer capture)', 'Parking Location',
        ...
        plt.savefig('cluster_performance.pdf')
        print("DONE GRAPHING AND SAVING PERFORMANCE\n")

        print("CREATING CSV GRAPH PERFORMANCE FILE")
        graph_performance_results = []
        for eps, smpl, perf, cluster in zip(x_values, y_values, performance, clusters):
            performance_values = (eps, smpl, perf, cluster)
        ...
        csv_graph_writer.writerow(('Epsilon', 'Min Samples', 'Total Performance', 'Number of Clusters'))
...
def main():
    ...
    dbscan = Cluster()
    dbscan.load_data(geofence_file='/Users/BrandonHall/Documents/GitHub/SUMDScrapeAndAnalysis'
                                   '/DC_Outlines/Washington_DC_Boundary/Washington_DC_Boundary.shp',
                     neighborhood_file='/Users/BrandonHall/Documents/GitHub/SUMDScrapeAndAnalysis/DC_Outlines/Roads/Roads.shp')
    epsilon = [0.003, 0.0003, 0.00003, 0.000003]

    dbscan.clustering(facilities_file='/Users/BrandonHall/Documents/GitHub/SUMDScrapeAndAnalysis'
                                      '/DC_Outlines/Washington_DC_Boundary/Washington_DC_BoundaryMultiPolygon.pkl',
                      csv_file='/Users/BrandonHall/Documents/GitHub/SUMDScrapeAndAnalysis/Clean_DC_CSVs/DC_TestData.csv',
                      epsilon=epsilon)
    # ***********************************************************
